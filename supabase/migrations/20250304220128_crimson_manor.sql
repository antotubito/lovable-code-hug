...
BEGIN
  ...
  -- Drop connection_requests policies
...
DO $$
BEGIN
  -- connection_codes indexes
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_codes_user_id_idx') THEN
    CREATE INDEX connection_codes_user_id_idx ON connection_codes(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_codes_scanned_by_idx') THEN
    CREATE INDEX connection_codes_scanned_by_idx ON connection_codes(scanned_by);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_codes_status_idx') THEN
    CREATE INDEX connection_codes_status_idx ON connection_codes(status);
  END IF;

  -- connection_requests indexes
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_requests_user_id_idx') THEN
    CREATE INDEX connection_requests_user_id_idx ON connection_requests(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_requests_requester_id_idx') THEN
    CREATE INDEX connection_requests_requester_id_idx ON connection_requests(requester_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'connection_requests_status_idx') THEN
