...
BEGIN
  ...
  DROP TRIGGER IF EXISTS handle_qr_code_expiration_trigger ON qr_codes;
  DROP TRIGGER IF EXISTS auto_generate_qr_code_trigger ON qr_codes;
  
  -- Drop existing functions
  DROP FUNCTION IF EXISTS handle_qr_code_expiration();
  DROP FUNCTION IF EXISTS auto_generate_qr_code();
  DROP FUNCTION IF EXISTS generate_qr_code();
  
  -- Drop existing policies
  DROP POLICY IF EXISTS "QR codes are readable by owner or scanner" ON qr_codes;
  DROP POLICY IF EXISTS "QR codes can be created by owner" ON qr_codes;
  DROP POLICY IF EXISTS "QR codes can be updated by owner or scanner" ON qr_codes;
